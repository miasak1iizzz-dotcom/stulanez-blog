> 历史快照（2026-09-09 整理前，包含当时未提交的约定）。仅用于追溯，不作新对话必读；现行规则见 [任务索引](../INDEX.md)。
...
**收工必须自行上线（老板 2026-09-07 明确，所有代理强制）：** 每干完一个**网站功能/代码/脚本**改动（或对站点/资产/协议的任何改动），负责的 AI 必须自己 `git push origin main` 推上线，让 Vercel 自动重新部署——改动不许留在本地工作区「等老板」或「等别人推」。这是硬规矩，不是可选项。推前按 §4 逐项暂存、只推本人认领范围；远端有新提交冲突时按 §4 处理，不强行变基、不推无关文件。

> **看板数据·本地优先（老板 2026-09-07 二次明确，覆盖上一条部分范围）：** 「AI 协作看板」是本地东西。看板数据——`src/data/agent-board/` 下的任务/卷宗/经验状态、`reports/<agentId>.json` 等记录协作进展的共享状态——**写进本地即可，不主动 push**；只有到了「上线展示/对外」、或它属于网站功能改动时，才推送。判断标准：改的是给别人看的网站功能 → 推；记的是咱们自己协作的状态 → 本地留档即可。
...
### AI 在线三态（老板 2026-09-07 定，所有 AI 强制维护心跳）

看板「AI 团队」显示三种状态——**工作中 / 空闲中 / 离线**：

- **离线**：该 AI 的客户端进程没在这台电脑上打开（看板自动探测，`agents.json` 的 `processNames`/路径映射，老板无需管）。
- **工作中 / 空闲中**：以**心跳**为准（看板读 `.ai-work/agent-presence.json`）。**每位 AI 开工时把心跳写成 `working`，收工（回完老板、停下来等下一步）时写成 `idle`**，用：
  `node scripts/agent-presence.mjs working|idle <bigmodel|deepseek|codex|cursor> [备注]`
  心跳超过 10 分钟没更新会自动当作停下（空闲）。没写心跳的 AI，看板回退到「有没有 doing 任务」推断。

> 硬规矩：**每个 AI 自己维护自己的心跳**，识别不了「正在干活」不代表可以不管——干完活必须标 `idle`，否则看板会一直显示工作中。文件在 `.ai-work/agent-presence.json`（本地、gitignore）。
