> 从原 CLAUDE.md 保存的架构参考（2026-09-09）。不是开工协议。命令以 package.json 为准，部署以 astro.config.mjs 和 [部署协议](../deployment.md) 为准；此文含旧构建步骤与静态部署描述，不据此回退代码。仅排查对应组件时读取相关章节。
...
---
历史原文到此结束。当前部署遵循 [部署协议](../deployment.md)。
