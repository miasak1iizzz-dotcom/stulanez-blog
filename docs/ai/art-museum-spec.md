...
- ✅ **阶段 0（2026-09-17 完成）**：清单格式定稿；判图管线 `scripts/art-scan.mjs` 跑通；`scripts/art-preview.mjs` 能把清单渲染成展墙预览页。
...
- **阶段 1**：艺术馆改读 manifest 渲染，游客参观视角上线（此时图还在本机，先验证体验）。
- **阶段 2**：开 R2 桶 + `img.stulanez.com` + 签名接口，打通浏览器直传。
- **阶段 3**：站长策展界面（选文件夹、批量打标、上下架）。
