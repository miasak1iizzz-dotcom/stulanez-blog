...
- T-028 看板重做 v2（bigmodel）：指挥室 UI + inbox 卷宗 + 新协作流程，自检中
- T-015 Serpent 链接资源库 + 全量重扫（**用户操作中**——库已重置干净，等链接后首扫 8.2 万文件）
- T-016 网站建设回归（**Codex 进行中**——下一项 T-018；额度恢复后继续）

## 最近完成（08-27 晚 ~ 09-04）

- 看板 v2 全面重做（T-028）：黑金指挥室、砍掉信箱/AI笔记/指南/已完成墙、inbox.json 卷宗制取代 decisions.json、board-enrichments 并入 tasks.json、协作协议看板章节重写（大白话规范+自查规则）
- 首页门户 Bento 回归（T-017）：8 张卡片与问候胶囊通过亮暗主题、375/414/768 断点、链接/背景图和控制台核验；无需源码修复
- kpop 目检轮 SEVENTEEN 批2（T-011）：600 张判定、134 张入隔离（22.3%）；批1 遗留 017 表 25 张补扫完成；管线修复（已判集+存在性过滤，E-019），下团 Kep1er
...
- **kpop 筛选（T-011）目检轮进行中**：保底清理收官（343 张已清）；SEVENTEEN 两批收官——共 1,175 张判定、330 张入 `资源库/_隔离待删-目检轮/`（批1 淘汰 34%、批2 淘汰 22.3%，待抽查后清空）；队列可续跑（Kep1er→OH MY GIRL→fromis_9…），全量约 2,100 表
- 二创返工标准（等删修样本）；二创「神图」标准仍开放（D-002）
...
1. Serpent 链接完成验收（用户）
2. 网站建设回归（Codex 进行中；下一项 T-018 首页文章卡与封面回归）
3. kpop 低质量图筛选（bigmodel，目检轮进行中：SEVENTEEN 已清完，Kep1er 接力）
