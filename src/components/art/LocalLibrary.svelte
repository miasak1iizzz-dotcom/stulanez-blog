<script lang="ts">
	...
	// 站长专属：本机图库浏览。原始全库在这里看，上站的那部分在 ArtGallery 展厅里。
...
<section class="art-museum" aria-label="我的图库">
	<header class="masthead">
		<div><span class="eyebrow">STULANEZ / LOCAL LIBRARY</span><h1>我的图库<span>本机目录 · 只在当前浏览器读取，原图不离开这台电脑。</span></h1></div>
		<div class="head-actions">
			{#if onExit}<button class="back" onclick={onExit}>← 返回展厅</button>{/if}
			{#if owner}
	...
	<footer>本机图库 · 私藏的另一种打开方式 <span>{connected ? "仅在当前浏览器读取；刷新后需重新选择目录。" : "相册与私人收藏，留给另一段故事。"}</span></footer>
</section>
...
<style>
	...
	.back { background: none; border: 1px solid color-mix(in srgb, var(--ink) 18%, transparent); color: var(--ink); border-radius: 7px; padding: 11px 15px; font-size: 12px; }
